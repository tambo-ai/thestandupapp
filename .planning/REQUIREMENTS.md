...
- [ ] **TEAM-01**: User can create a new team (workspace) which creates a WorkOS Organization
...
- [ ] **TEAM-04**: Invited user can accept invitation and join the team
...
- [ ] **DATA-01**: Multi-tenant schema with users, teams, memberships, and connection references in Turso
- [ ] **DATA-02**: All multi-tenant queries are scoped by team ID (no cross-tenant data leaks)
- [ ] **DATA-03**: User record stores WorkOS user ID as primary identifier

### Cross-Team AI

- [ ] **AI-01**: AI system prompt is rebuilt from server-side team and connection data (not localStorage)
- [ ] **AI-02**: AI tools use WorkOS Pipes token lookup per member for read operations
...
- [ ] **AI-05**: Write operations (create PR, open issue) use only the requesting user's connection
- [ ] **AI-06**: Each user retains their own personal conversation threads
